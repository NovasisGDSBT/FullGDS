    ...
    UINT32      reqDataSize)
{
    ...
    else
    {
        ...
        if (strstr(sessionData.requestInfo.fileName, DLEDS_FILE_EXTENSION_EDP2) != NULL)
        {
            sessionData.packageVersion = DLEDS_PACKAGE_VERSION_2;
        }
        else if (strstr(sessionData.requestInfo.fileName, DLEDS_FILE_EXTENSION_EDSP2) != NULL)
        {
            sessionData.packageVersion = DLEDS_PACKAGE_VERSION_2;
        }
...
#if 0   /* Temporarily remove to test if Package version 2 could be handled with protocol version 1 */
    if (sessionData.protocolVersion == DLEDS_PROTOCOL_VERSION_2)
    {
#endif
        if ((strstr(sessionData.requestInfo.fileName, DLEDS_FILE_EXTENSION_EDSP2) == NULL) &&
            (strstr(sessionData.requestInfo.fileName, DLEDS_FILE_EXTENSION_EDP2)  == NULL)   )
        {
            DebugError1("Not an EDP2 package file (%s)", sessionData.requestInfo.fileName);
            result = DL_ED_ERROR;
        }
