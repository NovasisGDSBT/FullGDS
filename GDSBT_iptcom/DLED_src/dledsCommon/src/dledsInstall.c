...
DLEDS_RESULT dledsUnpackEDPackage2(
    char* fileName,         /* IN: TAR file containing ED package to be installed */
    UINT8 edspNameSizeOf,   /* IN: Allocated memory size for ED package name */
    char* edspName)         /* OUT: Name of ED package */
{
    DLEDS_RESULT        result = DLEDS_OK;
    FILE*               fp;
    char                infoFile[256];
    ...
    char                sciName[64];
    char                sciFile[64];
    char                edspFileName[32];


    /* Check if it is an ED package version 2 */
    if ((strstr(fileName, DLEDS_FILE_EXTENSION_EDSP2) != NULL) ||
        (strstr(fileName, DLEDS_FILE_EXTENSION_EDP2)  != NULL)   )
    {
        /* Extract directory name from file name:  <EDSP_Name>.<extension> */
        pos = strchr(fileName, '.');
        ...
        /* Extract EDSP name from file name:  <EDSP_Name>.<extension> */
        pos = strchr(fileName, '.');
        strncpy(edspFileName, fileName, pos - fileName);
        edspFileName[pos - fileName] = '\0';
        ...
        if (strncmp(edspFileName, edspInfo.edspInfoHeader.edspName, strlen(edspInfo.edspInfoHeader.edspName)) != 0)
        {
            DebugError2("File name (%s) does not match EDSP name (%s) in EDSP_Info.xml", edspFileName, edspInfo.edspInfoHeader.edspName);
            setRootDirectory();
            return DLEDS_ERROR;
        }

        /* EDSP name in header info matches the file name */
        strncpy(edspName, edspInfo.edspInfoHeader.edspName, edspNameSizeOf-1);
        edspName[edspNameSizeOf-1] = '\0';  /* Make sure the string is NULL terminated */
    ...
    char* tempDir)               /* IN: Path to DLEDS temporary directory */
{
    ...
    {
        result = dledsUnpackEDPackage2(downloadFileName, 32, edspName);
